...
const PATH_CSV = path.resolve(__dirname, "ms_jabatan_fungsional.csv");
...
const buildMSJabatan = async (value) => {
  const data = {
    ...
    jabatan_bup: toInt(value["BUP"]),
    jabatan_jenjang: value["JENJANG"],
  };
...
const persistMSJabatan = async (jabatan) => {
  ...
  try {
    ...
    logger.error("[IMPORT] failed to import MS Jabatan", error);
    throw error;
  }
};
...
const importAllMSJabatan = async () => {
  try {
    ...
    logger.info(
      `[IMPORT] Finished importing MS Jabatan Fungsional. Total processed: ${processedCount}`,
    );
  } catch (error) {
...
importAllMSJabatan()
  .catch((error) => {
    logger.error(
      `[IMPORT MS Jabatan Fungsional] Unexpected failure: ${error.message}`,
    );
  })
  .finally(async () => {
